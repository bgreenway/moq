	...
	pub async fn run(self) -> anyhow::Result<()> {
		...
		match (&publish, &subscribe) {
			(Some(publish), Some(subscribe)) => {
				tracing::info!(transport, internal = token.internal, root = %token.root, publish = %publish.allowed().map(|p| p.to_string()).collect::<Vec<_>>().join(","), subscribe = %subscribe.allowed().map(|p| p.to_string()).collect::<Vec<_>>().join(","), "session accepted");
			}
			(Some(publish), None) => {
				tracing::info!(transport, internal = token.internal, root = %token.root, publish = %publish.allowed().map(|p| p.to_string()).collect::<Vec<_>>().join(","), "publisher accepted");
			}
			(None, Some(subscribe)) => {
				tracing::info!(transport, internal = token.internal, root = %token.root, subscribe = %subscribe.allowed().map(|p| p.to_string()).collect::<Vec<_>>().join(","), "subscriber accepted")
			}
			_ => {
